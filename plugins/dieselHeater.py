...
class heater:
    ...
    def createTransmitPacket(cls):
        try:
            ...
            buf[7] = transmitPacket.funSpeedMin.to_bytes(2, byteorder='big')[0].to_bytes(1, byteorder='big') #Minimum fan speed MSB, LSB
            buf[8] = transmitPacket.funSpeedMin.to_bytes(2, byteorder='big')[1].to_bytes(1, byteorder='big')
            buf[9] = transmitPacket.funSpeedMax.to_bytes(2, byteorder='big')[0].to_bytes(1, byteorder='big') #Maximum fan speed MSB, LSB
            buf[10] = transmitPacket.funSpeedMax.to_bytes(2, byteorder='big')[1].to_bytes(1, byteorder='big')
            ...
            buf[13] = 0x32.to_bytes(1, byteorder='big') if transmitPacket.thermostatMode == 1 else 0x32.to_bytes(1, byteorder='big') #Thermostat/Fixed mode, buf[3] = 0 when fixed mode
